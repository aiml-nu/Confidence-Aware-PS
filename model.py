...
class FeatureResNet_refine(ResNet):
    def __init__(self):
        super().__init__(BasicBlock, [3, 4, 6, 3], 1000)
        self.conv_f = conv(1+3+96,64,stride = 1)
...
class INInet(nn.Module):
    def __init__(self, pretrained_net):
        ...
        init.constant_(self.convd.weight, 0)  # Zero init
...
class CAPSNet(nn.Module):
    def __init__(self):
        ...
        self.feat = FeatureResNet_refine()
        self.relu = nn.ReLU(inplace=True)
        self.conv5 = conv(512, 256, stride=2, transposed=True)
        self.bn5 = bn(256)
        self.conv6 = conv(256, 128, stride=2, transposed=True)
        ...
        self.bnadd = bn(16)
        self.conv10 = conv(16, 3,stride=2, kernel_size=5)
        self.conv10_prob = conv(16, 1,stride=2, kernel_size=5)
        self.sigmoid = nn.Sigmoid()

        init.constant_(self.conv10.weight, 0)  # Zero init
        init.constant_(self.conv10_prob.weight, 0)  # Zero init

    def forward(self, x_1, x_2, x_img):
        x1, x2, x3, x4, x5 = self.feat(torch.cat([x_1, x_2, x_img], dim=1))
        x = self.relu(self.bn5(self.conv5(x5)))
        x = self.relu(self.bn6(self.conv6(x + x4)))
        x = self.relu(self.bn7(self.conv7(x + x3)))
        x = self.relu(self.bn8(self.conv8(x + x2)))
        x = self.relu(self.bn9(self.conv9(x + x1)))
        xn2 = self.relu(self.bnadd(self.convadd(x)))
        x_r1 = self.conv10(xn2) + x_1
        conf = self.sigmoid(self.conv10_prob(xn2))
        return x_1, x_2, x_r1, conf
